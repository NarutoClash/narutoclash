...
const StatBuff = ({ label, value }: { label: string; value: number }) => {
    ...
    return (
        <div className="flex justify-between items-center text-sm">
            <span className="text-[#888855]">{label}</span>
    ...
    return (
        <Card className={cn(
            ...
            <CardHeader>
                ...
                <CardDescription className="text-[#888855]">{weapon.description}</CardDescription>
            </CardHeader>
            <CardContent className="flex-grow space-y-4">
                 <div className="space-y-1">
                    <h4 className="font-semibold text-sm text-[#664400]">Atributos</h4>
                 ...
                 <div className="space-y-1">
                     ...
                     <p className="text-sm text-[#888855]">Nível: {weapon.requiredLevel}</p>
                 </div>
                 {weapon.passivas && weapon.passivas.length > 0 && (
                   <ItemPassivasDisplay passivas={weapon.passivas} mode="full" />
                 )}
            </CardContent>
    ...
    {isOwned ? (
        <AlertDialog>
          ...
          <AlertDialogContent className="bg-white border-[#ffcc00]/40">
            <AlertDialogHeader>
              <AlertDialogTitle className="text-[#664400]">Vender {weapon.name}?</AlertDialogTitle>
            ...
            <AlertDialogFooter>
              <AlertDialogCancel className="border-[#ffcc00] text-[#664400] hover:bg-[#ffcc00]/10">Cancelar</AlertDialogCancel>
              <AlertDialogAction 
...
export default function WeaponsPage() {
    ...
    const handleBuyWeapon = async (weapon: Weapon) => {
        ...
        const newRyo = (userProfile.ryo || 0) - weapon.price;
        ...
        if (error) {
            toast({ variant: 'destructive', title: 'Erro ao comprar arma', description: error.message });
            return;
        }
    ...
    const handleSellWeapon = async (weapon: Weapon) => {
        ...
        if (error) {
            toast({ variant: 'destructive', title: 'Erro ao vender arma', description: error.message });
            return;
        }

        if (updated) setUserProfile(updated);
        toast({ title: 'Arma Vendida!', description: `Você vendeu ${weapon.name} por ${sellPrice.toLocaleString()} Ryo.` });
    };
